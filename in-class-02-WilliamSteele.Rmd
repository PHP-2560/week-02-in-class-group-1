...
head(rain.df)
tail(rain.df)
# this function labels the first 3 columns as year, month, and day, and then proceeds to label the next 23 columns as a queue from 0:23. It is clear now that the last 24 columns represents the 24 hours in a day
...
head(rain.df, 25)  
hist(rain.df$daily)
#the problem is that there is negative rain...which is impossible
```
...
rain.df[rain.df$daily < 0 , ] = 0
rain.df$daily[which(rain.df$daily < 0)] = 0
rain.df.fixed = rain.df
#this takes all of the values that were -999 and sets them to NA as well as setting those daily values to 0
