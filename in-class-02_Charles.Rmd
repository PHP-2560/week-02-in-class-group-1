...
```{r}
library(dplyr)
rain.df <- read.table("rnf6080.dat") #loaded data table
...
```{r}
num_rows <- nrow(rain.df)
num_cols <- ncol(rain.df)

dim(rain.df)

# There are 27 columns and 5070 rows.
```
...
```{r}
names(rain.df)
# The names are the columns are V1, V2, ....V26, V27.
```
...
head(rain.df)
#This command named the columns of the data file. The first label, years, is the name of the first column and so on. The numbers represent the time during the day for which rainfall happened. 
...
hist(rain.df$daily)
# There is a negative value for the sum of the daily rainfall. This does not make sense because you can't have negative rainfall.
...
# to find the negative numbers: rain.df[rain.df$daily<0,]

# This is more reasonable because there are now no negative rainfall values. The x-axis starts at 0.
```
